...
// export data
void exportParticle(std::string fn, int size, double *px, double *py, double *pz,
...
#pragma omp parallel for
    ...
    fileName << "OA_" << std::setfill('0') << std::setw(3) << count << "_" <<
std::setfill('0') << std::setw(2) << myid << ".txt";
    exportParticle(fileName.str(), NUM_PARTICLES, x_cord, y_cord, z_cord, u_vel,
v_vel, w_vel);
    count++;
...
#pragma acc data copyin(x_cord [0:NUM_PARTICLES], y_cord [0:NUM_PARTICLES], z_cord
[0:NUM_PARTICLES], u_vel [0:NUM_PARTICLES], v_vel [0:NUM_PARTICLES], w_vel
[0:NUM_PARTICLES]) create(x_force [0:NUM_PARTICLES], y_force [0:NUM_PARTICLES],
z_force [0:NUM_PARTICLES])
    {
        for (int step = 1; step <= nSteps; step++)
        {
            const double tStart = tsecond(); // Start timing
            // zeros force
#pragma acc kernels present(x_force, y_force, z_force)
...
#pragma acc kernels present(x_cord, y_cord, z_cord, x_force, y_force, z_force)
#pragma acc loop independent
            for (int i = 0; i < NUM_PARTICLES; i++)
            {
...
#pragma acc loop independent
        ...
        {
            std::ostringstream fileName;
            fileName << "OA_" << std::setfill('0') << std::setw(3) << count <<
"_" << std::setfill('0') << std::setw(2) << myid << ".txt";
#pragma acc update self(x_cord [0:NUM_PARTICLES], y_cord [0:NUM_PARTICLES], z_cord
[0:NUM_PARTICLES], u_vel [0:NUM_PARTICLES], v_vel [0:NUM_PARTICLES], w_vel
[0:NUM_PARTICLES])
            exportParticle(fileName.str(), NUM_PARTICLES, x_cord, y_cord, z_cord,
u_vel, v_vel, w_vel);
            count++;
        }
        ...
        double mcz = 0.0;
        double maxSpeed = 0.0;
#pragma acc kernels present(x_cord, y_cord, z_cord, u_vel, v_vel, w_vel)
#pragma acc loop independent reduction(+ : mcx, mcy, mcz) reduction(max: maxSpeed)
        for (int i = 0; i < NUM_PARTICLES; i++)
        {
            mcx += x_cord[i];
            mcy += y_cord[i];
